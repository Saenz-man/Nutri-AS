...
interface ValidacionPlanProps {
  totales: { kcal: number; pro: number; lip: number; hco: number };
  metas: { kcal: number; proG: number; lipG: number; hcoG: number };
}

export default function ValidacionPlan({ totales, metas }: ValidacionPlanProps) {
  ...
  const getAdecuacion = (actual: number, meta: number) => {
    ...
    return { 
      porc: porc.toFixed(1), 
      color: isOk 
        ? "text-green-500 bg-green-50 border-green-100 shadow-green-100/50" 
        : "text-red-500 bg-red-50 border-red-100 shadow-red-100/50" 
    };
  };
  ...
  const indicadores = [
    ...
    { label: "HCO", actual: totales.hco, meta: metas.hcoG, unit: "g" },
  ];
    ...
    <section className="bg-white p-8 rounded-4xl border border-gray-100 shadow-sm">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {indicadores.map(res => {
          ...
          return (
            <div key={res.label} className={`p-5 rounded-3xl border-2 transition-all shadow-sm ${status.color}`}>
